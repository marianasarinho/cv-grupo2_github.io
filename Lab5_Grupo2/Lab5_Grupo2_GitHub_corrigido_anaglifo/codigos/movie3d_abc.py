...
def criar_anaglifo(left_rect, right_rect):
    """
    Anaglifo vermelho-ciano com lados invertidos em relacao ao primeiro teste.

    Nesta versao, a imagem direita fica no canal vermelho, e a imagem
    esquerda fica nos canais azul e verde. Isso corrige a percepcao 3D
    quando o par vermelho/ciano fica invertido nos oculos ou na montagem.
    """
    left_gray = cv2.cvtColor(left_rect, cv2.COLOR_BGR2GRAY)
    right_gray = cv2.cvtColor(right_rect, cv2.COLOR_BGR2GRAY)
    return cv2.merge([left_gray, left_gray, right_gray])
